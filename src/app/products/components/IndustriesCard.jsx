...
export default function IndustriesCard({ text, title, background }) {
  return (
    <div
      className={`${background} px-[4%] py-4 sm:py-7 mb-4 sm:mb-0 flex justify-end gap-7 flex-col h-full text-OtherWhite  text-[0.8rem] ssm:text-[0.85rem] xl:text-[0.9rem] leading-[214%] font-medium text-justify  `}
    >
      <div>
        <h3 className="text-[1.1rem] ssm:text-[1.2rem] md:text-[1.3rem] xl:text-[1.4rem] leading-[145%] font-semibold ">
          {title}
        </h3>
        <p className=" w-full md:w-[90%] lg:w-[80%] pt-3 xl:pt-4 ">{text}</p>
      </div>
